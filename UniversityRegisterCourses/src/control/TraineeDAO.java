...
//로그인 한 학생 수강 신청 전체 목록 (학번으로 검색)
	public ArrayList<TraineeVO> getTraineeTotalList(String sd_num) throws Exception {
		...
		// Connection.prepareStatement 메소드를 호출하여 PreparedStatement 오브젝트를 작성.
		// PreparedStatement.setXXX 메소드를 호출하여 입력 매개변수에 값을 전달.
		// PreparedStatement.executeQuery 메소드를 호출하여 ResultSet 오브젝트에서 SELECT문으로 결과 테이블을
		// 확보.
	...
	public ArrayList<TraineeVO> getTraineeTotalList() throws Exception {
		...
		String sql = "Select tr.no as no, tr.sd_num, le.l_name asl_num,st.sd_name as sd_name, t_section,t_date"
	...
	// 전체 수강 탭에서 학생이름 검색
	public ArrayList<TraineeVO> getTrainessStudentNameSearchList(String sd_name) throws Exception {
		ArrayList<TraineeVO> list = new ArrayList<>();
